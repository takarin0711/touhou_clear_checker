...
```sql
INSERT INTO characters (name) VALUES
('霊夢A（一点集中攻撃力重視型）'),       -- ホーミング安定型
('霊夢B（アンチパターン重視超誘導型）'),     -- 直線火力型
('魔理沙A（無限貫通＆常時攻撃型）'),     -- 前方集中型
('魔理沙B（超攻撃範囲重視型）'), -- 貫通特化
('早苗A（一点集中＆誘導型）'),       -- 一点集中・誘導型
('早苗B（高威力＆広範囲炸裂型）');       -- 高威力・広範囲爆発型
```
...
('霊夢B（妖器なし）'),      -- ホーミング針攻撃
('魔理沙A（ミニ八卦路）'),      -- 遠距離ダメージ低
('魔理沙B（妖器なし）'), -- 最強ファーミング性能
('咲夜A（シルバーブレード）'),      -- バリアボム・防御特化
('咲夜B（妖器なし）');        -- 咲夜A劣化版
```
